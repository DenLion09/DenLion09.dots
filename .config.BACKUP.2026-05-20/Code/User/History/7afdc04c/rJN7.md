...
**Responsabilidad:** Validar y sanitizar todo input de las herramiantas la modelo antes de procesarlo.
...
- **Bloquea** si se intenta usar librerías fuera del stack seleccionado y aprobado por el usuario
...
### 5.8 Chat Output Validator (NUEVO)

**Responsabilidad:** Validar específicamente los outputs que se envían al chat del usuario, asegurando que toda respuesta del Main y subagentes pase por filtros de seguridad antes de mostrarse.

#### Alcance

Este subsistema se enfoca en la **capa de presentación al usuario** — todo mensaje que el chat muestra debe pasar por aquí. No valida código (eso es Code Guardian), valida lo que el usuario ve/escucha.
...
2. Output Guard: filtra datos sensibles (keys, tokens, IPs)
     ↓
3. Security Scanner: detecta información sensible contextual
     ↓
4. Spec Enforcer: verifica que la respuesta cumple con el formato esperado
     ↓
5. Chat Output Validator: formatea para presentación (markdown, highlights)
     ↓
6. Frontend: muestra al usuario en el chat
```
...
3. Main: Analiza tarea, planifica estrategia
     ↓
4. SPEC ENFORCER: Carga spec para "auth/login"
     ↓
5. Main → ARQUITECTO (diseña solución)
     ↓
6. SPEC ENFORCER: Valida spec generada + STRACT.md
     ↓
7. Main → ejecutor (implementa código + STRACT.md y valida (test + lint))
     ↓
8. GUARDIAN: valida pasos de ejecucion y formatos de comunicacion
     ↓
9. SECURITY SCANNER: Scan vulnerabilidades
     ↓
10. DESIGN INSPECTOR: Valida estético y stack
     ↓
11. Main → REVISOR (revis calidad)
     ↓
12. OUTPUT GUARD: Filtra información sensible
     ↓
13. CHAT OUTPUT VALIDATOR: Formatea para chat
     ↓
14. Usuario: Respuesta final en chat
     ↓
15. Context Flush: Borra KV Cache
     ↓
16. Reload: Carga STRACT.md + grafo de tareas
