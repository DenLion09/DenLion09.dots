...
**Choice**: Estado global managed por estated patters con enums con transiciones discretas IDLE → LISTENING → THINKING → DELEGATING → STREAMING → ERROR
**Alternatives considered**: React Query + server state, callback hell, Redux
**Rationale**: El Chat tiene 7 estados bien definidos (per general_specs.md). Zustand ofrece la simplicidad de React Context con mejor rendimiento para actualizaciones frecuentes de streaming. La máquina de estados explícita permite debugging y testing claro.

### Decision: Provider Adapter Pattern para LLMs

**Choice**: Interfaz `LLMProvider` con implementaciones concretas para cada proveedor (OpenAI, Anthropic, Ollama, LM Studio)
...
│                    Context Manager                      │
│  ┌─────────────┐  ┌─────────────┐  ┌─────────────────┐  │
│  │ MessageBuf  │  │ STRACT.md   │  │ Git Log Cache   │  │
│  └─────────────┘  └─────────────┘  └─────────────────┘  │
└─────────────────────────────────────────────────────────┘
                            │
                            ▼
