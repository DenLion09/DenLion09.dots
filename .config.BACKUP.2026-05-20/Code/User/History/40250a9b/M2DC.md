...
**Choice**: Estado global manajado por el patron states con enums con transiciones discretas IDLE → LISTENING → THINKING → WORKING → DELEGATING → STREAMING → ERROR
...
| `src-tauri/src/chat/state.rs`                     | Create | Chat state machine              |
...
- [x] **Guardian Integration**: **DESCARTADO del MVP**. No es parte del alcance. Interface preparada pero no implementada.
