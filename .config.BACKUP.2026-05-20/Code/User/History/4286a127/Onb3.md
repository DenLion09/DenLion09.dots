...
const coche = {
  marca: "toyota",
  año: 2020,
  "propieddad-invalida": true, //comillas para nombres no valido
};
```

> NUNCA empieces un statament con {} - js lo interpreta como bloque, no como objeto.

#### Enjanced object Literales (ES6+)

```javascript
const obj = {
  __proto__: protoObj, //Prototipo
  handler, // Shorthand: handler: handler
  toString() {
    // Metodos shorthand
    return "hola";
  },
  ["prop_" + 42]: valor, // Propiedades compuradas
};
```
...
```javascript
const mi_str = "comillas simples";
const mi_seg_str = "comillas dobles";
const mi_ter_str = `templates literales`; // backticks - multilinea + interpolacion
```

##### **Templates literales** - La forma mderna:

```javascript
const nombre = "lev";
`Hola ${nombre}, ¿Cómo estás?`; // Interpolacicon
`Linea 1
Linea 2`; //multilinea
```
...
"una linea \n otra linea";
"c:\\ruta\\archivo"; //backslash escapado
