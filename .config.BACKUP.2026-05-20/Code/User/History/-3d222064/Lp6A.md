...
└─────────────────────────────────────────────────────────┘
     │     │                        │              │
   Menu  Tool Icons            Project name   Window ctrls
         (click = abrir)
```

- Iconos horizontalmente organizados en la barra superior
...
- Click en herramienta ya abierta = traer al frente / cambiar tab activo
- Click derecho en icono = contexto (cerrar, minimizar, etc.)

### 4.2 Toolbar Secundaria (Tabs del Workspace)

```
┌─────────────────────────────────────────────────────────┐
│  Chat │ Editor ˅│ Browser ˅│ Terminal ˅│     [+] │ → │
└─────────────────────────────────────────────────────────┘
```

- Pestañas de herramientas abiertas, debajo de la toolbar principal
- Arrastrar y soltar para reordenar
- Click derecho → Layout (split horizontal, split vertical, tab)
- Botón `[+]` para abrir nueva herramienta (lanza TOOL_SELECT)
- Botón `→` para scroll cuando hay muchas tabs
...
| :---------------------- | :----------------------------------- | :----------------------------- |
| **Abrir herramienta**   | Click en icono toolbar o [+] en tabs | Nueva herramienta en workspace |
| **Cerrar herramienta**  | Click ✕ en tab                       | Cierra herramienta             |
| **Cambiar herramienta** | Click en tab                         | Cambia herramienta activa      |
| **Cambiar layout**      | Clic derecho en tab → Layout         | Cambia disposición             |
| **Reordenar tabs**      | Drag & drop en tabs                  | Cambia orden de pestañas       |
...
```
┌────────────────────────────────────────────────────────────┐
│  ≡      │  Proyecto: mi-app    [▣] [▣] [▣] [▣]  │ ─  □  ✕  │  ← Toolbar Principal
├────────────────────────────────────────────────────────────┤
│                                                            |
                                                            |
│                                                            │
...
│                                                            │
├────────────────────────────────────────────────────────────┤
│ errors[] warnings [] │  RAM: 45MB  │  Agent: IDLE  │  v1.0.0              │  ← Status Bar
└────────────────────────────────────────────────────────────┘
   │          │               │
