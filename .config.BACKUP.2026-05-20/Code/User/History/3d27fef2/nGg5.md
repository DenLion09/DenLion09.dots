...
Este documento consolida las especificaciones técnicas de cada aplicación de desarrollo propio del proyecto, organizadas por fases de implementación: **MVP (Minimum Viable Product)**, **Intermedia** y **Avanzada**.

Las aplicaciones se clasifican según el roadmap definido en `general_specs.md` y se complementan con referencias a las specs detalladas en el directorio `specs/`.

**Convenciones:**

- Las features marcadas como **"sin especificar"** indican que no existe documentación técnica detallada en las specs disponibles
- Los términos técnicos en inglés se mantienen para facilitar la búsqueda en documentación internacional
- Las referencias a specs usan el formato `specs/XX-nombre.md`
...
| **Estados UI**               | IDLE, LISTENING, THINKING, DELEGATING, WORKING, STREAMING, ERROR                                       |
| **Animación Ciri**           | Indicador visual de estado inspirado en señales orgánicas/fluidas                                      |
...
| **Input Enhancer**           | Corrección ortográfica antes de enviar al Guardian (NO es validación de seguridad)                     |
...
4. **Ranking Final**: semantic_similarity _ 0.5 + stack_overlap _ 0.2 + success_rate _ 0.2 + log(usage_count) _ 0.1
